...
export function AccountingStats({ stats, filterType, onFilterChange }: AccountingStatsProps) {
    ...
    return (
        <div className="grid grid-cols-3 gap-4 mb-4">
            <div
                className={`bg-card rounded-lg border p-4 cursor-pointer transition-colors ${filterType === "pending" ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}
                onClick={() => handleFilterClick("pending")}
            >
                <div className="text-sm text-muted-foreground break-all">{t("accounting.stats.pending")}</div>
                <div className="text-2xl font-bold text-warning mt-2">{stats.waitingExportInvoiceCount || 0}</div>
            </div>

            <div
                className={`bg-card rounded-lg border p-4 cursor-pointer transition-colors ${filterType === "completed" ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}
                onClick={() => handleFilterClick("completed")}
            >
                <div className="text-sm text-muted-foreground break-all">{t("accounting.stats.completed")}</div>
                <div className="text-2xl font-bold text-success mt-2">{stats.paymentProblemOrderCount || 0}</div>
            </div>

            <div
                className={`bg-card rounded-lg border p-4 cursor-pointer transition-colors ${filterType === "totalPending" ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}
                onClick={() => handleFilterClick("totalPending")}
            >
                <div className="text-sm text-muted-foreground break-all">{t("accounting.stats.totalPendingValue")}</div>
