    ...
    <TooltipProvider delayDuration={150}>
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="truncate font-mono text-md text-foreground/80">
            {inline}
          </div>
        </TooltipTrigger>
        <TooltipContent className="max-w-[720px] p-0 text-md">
          <pre className="p-3 font-mono max-h-[50vh] overflow-auto">
            {pretty}
          </pre>
        </TooltipContent>
      </Tooltip>
...
function ExcelFilterDropdown({
  ...
  const appliedValues = filters[columnKey]?.values ?? [];
  const [draftValues, setDraftValues] = useState<string[]>(appliedValues);
  const [search, setSearch] = useState("");

  const allValues = useMemo(
    () =>
      Array.from(new Set(rows.map((r) => String(r[columnKey] ?? "")))).sort(),
    [rows, columnKey]
  );
  ...
  return (
    <div className="w-60 p-2 space-y-2 text-md">
      ...
      <label className="flex items-center gap-2 text-md">
        <input
          type="checkbox"
          checked={draftValues.length === allValues.length}
          onChange={toggleAll}
        />
        (Select All)
      </label>
      ...
      <div className="max-h-48 overflow-auto space-y-1">
        {visibleValues.map((v) => (
          <label
            ...
            <input
              type="checkbox"
              checked={draftValues.includes(v)}
...
function DateRangeFilterDropdown({
  ...
  const rangeOk =
    !fromDate || !toDate
      ? true
      : !isAfter(startOfDay(fromDate), startOfDay(toDate)); // from <= to
  ...
  return (
    <div className="w-80 p-3 space-y-3 text-md">
      ...
      {touched && fromOk && toOk && !rangeOk && (
        <div className="text-xs text-destructive">
          {t("dashboardLogs.invalidDateRange")}
        </div>
      )}
              ...
              <tr>
                {columns.map((col) => (
                  <th
                    key={col}
                    className={cn(
                      "p-3 text-left align-top text-lg",
                      isFiltered(col) &&
                        (col === "command"
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {col !== "data" && col !== "id" && (
                        <DropdownMenu
                          open={openCol === col}
                          onOpenChange={(v) => setOpenCol(v ? col : null)}>
              ...
              {filteredRows.map((r) => (
                <tr
                  ...
                  <td
                    className={cn(
                      "p-3 font-mono",
                      isFiltered("id") && filteredColClass
                    )}>
                    {r.id}
                  </td>
                  <td
                    className={cn(
                      "p-3 font-mono",
                      isFiltered("createdAt") && filteredColClass
                    )}>
                    {r.createdAt}
                  </td>
                  <td
                    className={cn(
                      "p-3 font-mono",
                      isFiltered("createdById") && filteredColClass
                    )}>
                    {r.createdById}
                  </td>
                  <td
                    className={cn(
                      "p-3",
                      isFiltered("command") && filteredColClass
                    )}>
                    <Badge className={commandBadgeClass(r.command)}>
                      {t(`dashboardLogs.command.${r.command}`)}
                    </Badge>
                  </td>
                  <td
                    className={cn(
                      "p-3 font-mono",
                      isFiltered("sourceTable") && filteredColClass
                    )}>
                    {r.sourceTable}
                  </td>
                  <td
                    className={cn(
                      "p-3 font-mono",
                      isFiltered("sourceIds") && filteredColClass
                    )}>
                    {r.sourceIds}
                  </td>
                  ...
                  <td
                    className={cn(
                      "p-3 text-md text-muted-foreground",
                      isFiltered("intent") && filteredColClass
                    )}>
                    {r.intent}
                  </td>
                </tr>
