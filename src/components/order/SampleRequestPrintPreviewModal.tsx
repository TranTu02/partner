        ...
        <div className="fixed inset-0 z-[100] flex flex-col bg-background animate-in fade-in duration-200">
            ...
            <div className="h-14 border-b border-border flex items-center justify-between px-4 bg-card shrink-0 shadow-sm">
                <div className="flex items-center gap-4">
                    <img src="https://documents-sea.bildr.com/rc19670b8d48b4c5ba0f89058aa6e7e4b/doc/IRDOP%20LOGO%20with%20Name.w8flZn8NnkuLrYinAamIkw.PAAKeAHDVEm9mFvCFtA46Q.svg" alt="Logo" className="h-8" />
                    <div>
            ...
            <div className="flex-1 flex overflow-hidden bg-gray-50/50 relative">
                                        ...
                                        <div className="mt-3 grid grid-cols-2 gap-2 p-2 bg-muted/20 rounded-md border border-border/50">
                                            {sample.sampleInfo.map((info, iIdx) => (
                                                <Field 
                                                    ...
                                                    label={info.label} 
                                                    value={info.value} 
                                                    onChange={v => handleUpdateSampleInfo(sIdx, iIdx, v)} 
                ...
                {/* Mobile Guide Button */}
                <button onClick={() => setShowGuide(!showGuide)} className="fixed bottom-6 right-6 xl:hidden z-50 p-3 bg-primary text-primary-foreground rounded-full shadow-2xl hover:scale-105 active:scale-95 transition-transform">
                ...
                {showGuide && (
                    <div className="fixed inset-0 z-[110] bg-black/40 backdrop-blur-sm flex items-end justify-center px-4 pb-4 animate-in slide-in-from-bottom duration-300">
                        <div className="bg-white rounded-2xl w-full max-w-lg p-6 relative overflow-hidden">
                            <button onClick={() => setShowGuide(false)} className="absolute top-4 right-4 text-muted-foreground"><X className="w-5 h-5"/></button>
                            <GuideContent />
                        </div>
                    </div>
                )}
...
function Section({ title, children, color }: { title: string; children: any; color: string }) {
    ...
    return (
        <div className="bg-card rounded-xl border border-border p-4 shadow-sm relative pt-6 animate-in fade-in duration-500">
            <div className={`absolute top-0 left-0 px-2.5 py-0.5 rounded-br-lg text-[9px] font-black uppercase tracking-widest ${colors[color] || colors.blue}`}>
                {title}
            </div>
        ...
        .map((sample, sampleIdx) => {
            ...
            const rowsHtml = analyses
                .map((analysis: any, index: number) => {
                    ...
                    // Build all sampleInfo lines including "Tên mẫu thử"
                    const allSampleInfo = sample.sampleInfo && sample.sampleInfo.length > 0
                        ? sample.sampleInfo
                        : [{ label: "Tên mẫu thử", value: sample.sampleName || "" }];
