export default {
    translation: {
        common: {
            close: "Đóng",
            cancel: "Hủy",
            save: "Lưu",
            saving: "Đang lưu...",
            saveSuccess: "Lưu thành công",
            delete: "Xóa",
            confirm: "Xác nhận",
            search: "Tìm kiếm",
            select: "Chọn",
            filter: "Bộ lọc",
            status: "Trạng thái",
            action: "Hành động",
            actions: "Hành động",
            preview: "Xem trước",
            exportPDF: "Xuất PDF",
            unsavedChangesMessage: "Bạn có thay đổi chưa lưu. Chắc chắn tiếp tục?",
            print: "In",
            printWithoutSaving: "In không lưu",
            saveAndPrint: "Lưu và in",
            details: "Chi tiết",
            edit: "Chỉnh sửa",
            add: "Thêm mới",
            loading: "Đang tải...",
            error: "Đã có lỗi xảy ra",
            success: "Thành công",
            yes: "Có",
            no: "Không",
            export: "Xuất file",
            saveChanges: "Lưu thay đổi",
            unsavedChanges: "Bạn có thay đổi chưa lưu. Bạn có chắc muốn thoát?",
            ifAny: "nếu có",
            page: "Trang",
            exportPdf: "Xuất PDF",
            view: "Xem",
            duplicate: "Nhân bản",
            parameter: "Chỉ tiêu",
        },
        pagination: {
            showing: "Hiển thị",
            to: "đến",
            of: "trong tổng số",
            results: "kết quả",
            previous: "Trang trước",
            next: "Trang sau",
            rowsPerPage: "Số hàng mỗi trang",
        },
        dashboardLogs: {
            title: "Nhật ký hệ thống",
            header: "Trang quản trị",
            subtitle: "Theo dõi các thao tác quản trị",
            from: "Ngày bắt đầu",
            to: "Ngày kết thúc",
            clear: "Xóa",
            ok: "Lọc",

            filters: {
                schema: "Schema",
                command: "Hành động",
                search: "Tìm kiếm",
            },

            empty: "Không có bản ghi nào",
            invalidDate: "Không đúng định dạng (YYYY-MM-DD)",
            invalidDateRange: "Ngày kết thúc không sớm hơn ngày bắt đầu",

            columns: {
                id: "ID",
                createdAt: "Thời gian",
                createdById: "Người thao tác",
                command: "Hành động",
                sourceTable: "Bảng nguồn",
                sourceIds: "ID nguồn",
                data: "Dữ liệu",
                intent: "Mục đích",
            },

            command: {
                CREATE: "TẠO MỚI",
                UPDATE: "CẬP NHẬT",
                DELETE: "XÓA",
                READ: "XEM",
            },

            schema: {
                identity: "Identity",
                crm: "CRM",
                library: "Thư viện",
                lab: "Phòng thí nghiệm",
                document: "Tài liệu",
                service: "Dịch vụ",
            },
        },
        client: {
            details: "Chi tiết khách hàng",
            clientId: "Mã khách hàng",
            code: "Mã KH",
            basicInfo: "Thông tin cơ bản",
            name: "Tên khách hàng",
            taxCode: "Mã số thuế",
            address: "Địa chỉ",
            invoiceInfo: "Thông tin hóa đơn",
            copyBasicInfo: "Lấy thông tin cơ bản",
            invoiceEmail: "Email hóa đơn",
            invoiceDetails: "Thông tin xuất hóa đơn",
            businessStats: "Thống kê kinh doanh",
            totalRevenue: "Tổng doanh thu",
            lastOrder: "Đơn hàng cuối",
            accessControl: "Quyền truy cập",
            identityIds: "Identity IDs có quyền truy cập",
            contacts: "Người liên hệ",
            noContacts: "Chưa có người liên hệ nào",
            management: "Quản lý Khách hàng",
            subtitle: "Danh sách khách hàng và thông tin liên hệ",
            searchPlaceholder: "Tìm kiếm theo tên hoặc mã số thuế...",
            noClientsFound: "Không tìm thấy khách hàng nào",
            confirmDelete: "Bạn có chắc chắn muốn xóa khách hàng này?",
            contactPerson: "Người liên hệ",
            contactPhone: "Số điện thoại liên hệ",
            contactIdentity: "Căn cước công dân",
            reportEmail: "Email báo cáo",
            contactInfo: "Thông tin liên hệ",
            contactEmail: "Email liên hệ",
            contactAddress: "Địa chỉ liên hệ",
            position: "Chức vụ",
            phone: "Số điện thoại",
            email: "Email",
            taxName: "Tên công ty (HĐ)",
            taxNamePlaceholder: "Nhập tên công ty xuất hóa đơn",
            taxCodePlaceholder: "Nhập mã số thuế",
            taxAddress: "Địa chỉ (HĐ)",
            taxAddressPlaceholder: "Nhập địa chỉ xuất hóa đơn",
            taxEmail: "Email nhận hóa đơn",
            taxEmailPlaceholder: "Nhập email nhận hóa đơn",
            accessScope: "Phạm vi truy cập",
            scope: {
                public: "Public",
                private: "Private",
            },
            add: {
                title: "Thêm khách hàng mới",
                submit: "Thêm khách hàng",
            },
            edit: {
                title: "Chỉnh sửa khách hàng",
                note: "Lưu ý: Không thể thay đổi Mã khách hàng sau khi đã tạo. Phạm vi truy cập Private sẽ giới hạn khách hàng này chỉ hiển thị với những người dùng được chỉ định.",
            },
        },
        sample: {
            list: "Danh sách mẫu",
            name: "Thông tin mẫu",
            matrix: "Nền mẫu",
            note: "Ghi chú",
            desc: "Mô tả",
        },
        table: {
            stt: "STT",
            parameter: "Chỉ tiêu",
            method: "Phương pháp",
            unitPrice: "Đơn giá",
            vat: "VAT",
        },
        analysis: {
            selectParameters: "Chọn chỉ tiêu phân tích",
            searchPlaceholder: "Tìm theo tên chỉ tiêu hoặc mã phương pháp...",
            filterAll: "Tất cả",
            parameterName: "Chỉ tiêu",
            protocol: "Phương pháp",
            field: "Lĩnh vực",
            unitPrice: "Đơn giá",
            noParametersFound: "Không tìm thấy chỉ tiêu nào phù hợp",
            selectedCount: "Đã chọn: <1>{{count}}</1> chỉ tiêu",
            confirmAdd: "Xác nhận thêm ({{count}}) chỉ tiêu",
            fields: {
                chemistry: "Hóa học",
                physics: "Lý học",
                microbiology: "Vi sinh",
            },
        },
        parameter: {
            management: "Quản lý chỉ tiêu phân tích",
            subtitle: "Danh sách chỉ tiêu",
            add: "Thêm chỉ tiêu",
            name: "Tên chỉ tiêu",
            protocol: "Phương pháp",
            field: "Lĩnh vực",
            unitPrice: "Đơn giá",
            tax: "Thuế (%)",
            protocolNote: "Chọn từ danh sách phương pháp có sẵn",
            action: "Thao tác",
            group: "Nhóm chỉ tiêu",
            groupName: "Tên nhóm",
            note: "Ghi chú",
            noParameters: "Không tìm thấy chỉ tiêu nào",
            confirmDelete: "Bạn có chắc chắn muốn xóa chỉ tiêu này?",
            selectTitle: "Chọn chỉ tiêu phân tích",
            selectFromList: "Chọn từ danh sách",
            selectFromGroup: "Gói kiểm chuyên sâu",
            bulkInput: "Nhập hàng loạt",
            discountRate: "Giảm (%)",
            bulkLabel: "Nhập danh sách chỉ tiêu (phân tách bằng dấu ';')",
            bulkPlaceholder: "Ví dụ: pH; COD; BOD5; TSS; Ammonium",
            searchPlaceholder: "Tìm theo tên chỉ tiêu hoặc mã phương pháp...",
            searchButton: "Tìm kiếm chỉ tiêu",
            allFields: "Tất cả",
            notFound: "Không tìm thấy chỉ tiêu nào phù hợp",
            selectedCount: "Đã chọn",
            cancel: "Hủy",
            confirmAdd: "Xác nhận thêm ({{count}}) chỉ tiêu",
            alertInput: "Vui lòng nhập danh sách chỉ tiêu!",
            enterSampleTypeFirst: "Vui lòng nhập hoặc chọn nền mẫu trước",
            noParametersSelected: "Chưa có chỉ tiêu nào được chọn",
            sumUnitPrice: "Tổng đơn giá",
            totalDiscount: "Chiết khấu",
            sumBeforeTax: "Tiền trước thuế",
            sumAfterTax: "Tổng tiền",
        },
        sidebar: {
            dashboard: "Bảng điều khiển",
            clients: "Khách hàng",
            quotes: "Báo giá",
            orders: "Đơn hàng",
            parameters: "Chỉ tiêu",
            accounting: "Kế toán",
            settings: "Cài đặt",
            admin: "Quản trị hệ thống",
            logout: "Đăng xuất",
            instituteName: "Viện nghiên cứu và phát triển<br />sản phẩm thiên nhiên",
            guest: "Khách vãng lai",
            limitedAccess: "Truy cập hạn chế",
            reports: "Báo cáo",
            analytics: "Phân tích",
        },
        roles: {
            superAdmin: "Quản trị hệ thống",
            director: "Giám đốc PTN",
            techManager: "Quản lý Kỹ thuật",
            qaManager: "Quản lý Chất lượng",
            sectionHead: "Trưởng bộ phận",
            validator: "Kiểm soát viên",
            seniorAnalyst: "Kiểm nghiệm viên chính",
            technician: "Kiểm nghiệm viên",
            ipcInspector: "Nhân viên IPC",
            rndSpecialist: "Nhân viên R&D",
            receptionist: "Nhân viên nhận mẫu",
            sampler: "Nhân viên lấy mẫu",
            sampleCustodian: "Thủ kho mẫu",
            equipmentMgr: "Quản lý thiết bị",
            inventoryMgr: "Quản lý hóa chất",
            salesManager: "Trưởng phòng kinh doanh",
            salesExec: "Nhân viên kinh doanh",
            cs: "Chăm sóc khách hàng",
            accountant: "Kế toán",
            reportOfficer: "Admin báo cáo",
            docController: "Kiểm soát tài liệu",
            hseOfficer: "An toàn & Môi trường",
            collaborator: "Cộng tác viên",
            Collaborator: "Cộng tác viên",
            qualityControl: "Kiểm soát chất lượng",
            administrative: "Hành chính",
        },
        order: {
            management: "Quản lý đơn hàng",
            subtitle: "Danh sách các đơn hàng kiểm nghiệm",
            create: "Tạo đơn hàng mới",
            exportSampleRequest: "Phát hành Phiếu gửi mẫu",
            searchPlaceholder: "Tìm kiếm theo mã đơn hàng, mã báo giá hoặc tên khách hàng...",
            noOrdersFound: "Không tìm thấy đơn hàng nào",
            code: "Mã đơn hàng",
            quoteCode: "Mã báo giá",
            client: "Khách hàng",
            total: "Tổng thanh toán",
            status: "Trạng thái đơn",
            paymentStatus: "Thanh toán",
            createdDate: "Ngày tạo",
            statuses: {
                pending: "Chờ xử lý",
                processing: "Đang xử lý", // Fallback or additional state
                approved: "Đã duyệt",
                cancelled: "Hủy bỏ",
                completed: "Hoàn thành",
            },
            salePerson: "Chăm sóc khách hàng",

            paymentStatuses: {
                unpaid: "Chưa thanh toán",
                partial: "Thanh toán 1 phần",
                mismatch: "Thanh toán không khớp",
                debt: "Công Nợ",
                paid: "Đã thanh toán",
                variance: "Lệch thanh toán",
            },
            detail: "Chi tiết đơn hàng",
            detailTitle: "Chi tiết đơn hàng",
            detailTotalValue: "Tổng giá trị đơn hàng",

            sampleName: "Tên mẫu",
            sampleMatrix: "Nền mẫu",
            sampleNamePlaceholder: "Ví dụ: Nước thải sau xử lý",
            sampleMatrixPlaceholder: "Ví dụ: Nước",
            duplicateSample: "Nhân bản mẫu",
            duplicateCount: "Số lượng nhân bản",
            removeSample: "Xóa mẫu",
            addAnalysis: "Thêm chỉ tiêu",
            noAnalyses: "Chưa có chỉ tiêu nào. Nhấn 'Thêm chỉ tiêu' để bắt đầu.",
            lineTotal: "Thành tiền(VND)",
            subtotal: "Tạm tính",
            discount: "Chiết khấu",
            vat: "Thuế VAT",
            pricing: {
                title: "Tổng kết chi phí",
                discountAmount: "Tiền giảm",
                discountMoney: "Tiền chiết khấu",
                commission: "Hoa hồng Sale",
                commissionAmount: "Tiền hoa hồng",
                feeBeforeTax: "Tổng tiền trước thuế",
            },
            information: "Thông tin đơn hàng",
            quoteCodePlaceholder: "Nhập mã báo giá (ví dụ: QT-20241230-01)",
            loadQuote: "Tải báo giá",
            quoteNote: "Để trống nếu đây là đơn hàng mới không xuất phát từ báo giá. Nhập mã và bấm 'Tải báo giá' để tự động điền thông tin.",
            samples: "Mẫu phân tích",
            sampleList: "Danh sách mẫu",
            note: "Ghi chú",
            notePlaceholder: "Nhập ghi chú...",
            addSample: "Thêm mẫu mới",
            otherItems: {
                title: "Phụ phí",
                itemName: "Loại phí",
                feeBeforeTax: "Tiền trước thuế",
                taxRate: "Thuế (%)",
                feeAfterTax: "Tiền sau thuế",
                addItem: "Thêm mục phí",
                removeItem: "Xóa",
                noItems: "Chưa có phụ phí nào",
                quickSelect: "Chọn nhanh",
                presets: {
                    fast: "Phí làm nhanh",
                    standard: "Phí xây dựng tiêu chuẩn cơ sở",
                    hardcopy: "Phí phát hành bản cứng",
                },
            },
            errorQuoteCode: "Vui lòng nhập mã báo giá!",
            loadQuoteSuccess: "Đã tải thành công dữ liệu từ báo giá!",
            quoteNotFound: "Không tìm thấy báo giá với mã này!",
            saveSuccess: "Đơn hàng đã được lưu thành công!",
            printButton: "In Đơn hàng",
            customerAttachments: "Tài liệu đính kèm từ khách hàng",
            addAttachment: "Thêm tài liệu",
            noAttachments: "Chưa có tài liệu đính kèm",
            loadingFileName: "Đang tải tên file...",
            attachmentNote: "* Đính kèm các hồ sơ liên quan đến đơn hàng (Bản scan phiếu gửi mẫu có đóng dấu, COA, hóa đơn...).",
            print: {
                order: "Đơn hàng",
                orderId: "Mã đơn hàng",
                sampleRequest: "Phiếu gửi mẫu",
                previewTitle: "Xem trước Đơn Hàng",
                title: "ĐƠN HÀNG",
                sample: "Mẫu",
                stt: "STT",
                parameter: "Chỉ tiêu",
                method: "Phương pháp",
                unitPrice: "Đơn giá(VND)",
                amount: "Thành tiền",
                tax: "Thuế",
                quantity: "SL",
                discount: "Chiết khấu toàn đơn(VND)",
                provider: "BÊN CUNG CẤP DỊCH VỤ",
                unit: "Đơn vị",
                address: "Địa chỉ",
                phone: "Điện thoại",
                clientInfo: "THÔNG TIN KHÁCH HÀNG",
                client: "KHÁCH HÀNG",
                customer: "Khách hàng",
                taxCode: "Mã số thuế/CCCD",
                contact: "Người liên hệ",
                email: "Email nhận kết quả",
                reportEmail: "Email báo cáo",
                samplesAndAnalysis: "CHI TIẾT ĐƠN HÀNG",
                total: "TỔNG CỘNG (VNĐ)",
                subtotal: "Tổng đơn giá toàn bộ mẫu",
                vat: "Thuế VAT",
                grandTotal: "Tổng tiền thanh toán (đã bao gồm VAT)",
                clientRep: "Đại diện khách hàng",
                companyRep: "Đại diện công ty",
                signName: "Ký, đóng dấu, ghi rõ họ tên",
                header: {
                    lines: {
                        line1: "VIỆN NGHIÊN CỨU VÀ PHÁT TRIỂN SẢN PHẨM THIÊN NHIÊN",
                        line2: "Địa chỉ: 12 Phùng Khoang 2, P. Đại Mỗ, TP. Hà Nội - Điện thoại: 024 355 35 355 - Email: Cskh@irdop.org",
                        line3: "",
                    },
                },
                notePrefix: "Ghi chú:",
                amountInWords: "Bằng chữ",
                salePerson: "Người liên hệ",
                validityNote: "Đơn hàng có giá trị tại thời điểm xuất đơn: ",
                disclaimer: "Thời gian thử nghiệm tính từ lúc Bên cung cấp dịch vụ nhận được mẫu kiểm nghiệm kèm hồ sơ Hợp lệ của Khách hàng và nhận được chi phí.",
                bankInfo: {
                    title: "Thông tin chuyển khoản:",
                    accountName: "Viện nghiên cứu và phát triển sản phẩm thiên nhiên",
                    accountNumber: "Số tài khoản: 16356688",
                    bankName: "Ngân hàng ACB - chi nhánh Hà Nội",
                    transferContent: "Nội dung chuyển khoản",
                },
                customerContent: "< Thêm thông tin khách hàng nếu cần>",
                customerConfirmation: "Xác nhận của khách hàng",
            },
        },
        quote: {
            management: "Quản lý báo giá",
            subtitle: "Danh sách các báo giá đã tạo",
            create: "Tạo báo giá mới",
            searchPlaceholder: "Tìm kiếm theo mã báo giá hoặc tên khách hàng...",
            noQuotesFound: "Không tìm thấy báo giá nào",
            code: "Mã báo giá",
            client: "Khách hàng",
            total: "Tổng thanh toán",
            status: "Trạng thái",
            createdDate: "Ngày tạo",
            statuses: {
                draft: "Nháp",
                converted: "Đã lên đơn",
                pending: "Chờ xử lý",
                final: "Chốt",
                expired: "Hết hạn",
            },
            detail: "Chi tiết báo giá",
            printButton: "In Báo giá",
            print: {
                validityNote: "Báo giá có giá trị trong vòng 15 ngày kể từ ngày báo giá:",
                title: "BÁO GIÁ",
                previewTitle: "Xem trước Báo giá",
            },
        },
        accounting: {
            management: "Quản lý hóa đơn",
            subtitle: "Danh sách đơn hàng đã hoàn thành chưa xuất hóa đơn",
            searchPlaceholder: "Tìm kiếm theo mã đơn hàng hoặc tên khách hàng...",
            noInvoicesFound: "Không có đơn hàng nào cần xuất hóa đơn",
            stats: {
                pending: "Chưa xuất hóa đơn",
                completed: "Lệch/Chờ thanh toán",
                totalPendingValue: "Tổng giá trị lệch/chờ",
            },
            table: {
                orderCode: "Mã đơn hàng",
                orderId: "Mã đơn hàng",
                receiptId: "Mã tiếp nhận",
                client: "Khách hàng",
                taxCode: "Mã số thuế",
                invoiceNumbers: "Số hóa đơn",
                total: "Tổng giá trị đơn",
                totalPaid: "Đã thanh toán",
                date: "Ngày tiếp nhận",
                completedDate: "Ngày hoàn thành",
                salePerson: "Người tạo",
                status: "Trạng thái",
                payment: "Thanh toán",
            },
            createInvoice: "Tạo hóa đơn",
            edit: "Chỉnh sửa",
            orderDetail: "Chi tiết đơn hàng",
            transactions: "Lịch sử giao dịch",
            amount: "Số tiền",
            date: "Ngày",
            method: "Phương thức",
            note: "Ghi chú",
            noTransactions: "Chưa có giao dịch nào",
            confirmInvoice: "Xác nhận xuất hóa đơn",
            successMessage: "Hóa đơn cho đơn hàng {{orderId}} đã được tạo thành công!",
            invoices: "Hóa đơn",
            totalPaid: "Tổng đã thanh toán",
            filter: {
                fromDate: "Từ ngày",
                toDate: "Đến ngày",
                apply: "Áp dụng",
                clear: "Hủy",
                paymentFilter: "Trạng thái thanh toán",
                hasPaid: "Có thanh toán",
                notPaid: "Chưa thanh toán",
                paymentDateFilter: "Lọc theo ngày thanh toán",
                amountFilter: "Lọc theo giá trị đơn",
                fromAmount: "Từ số tiền",
                toAmount: "Đến số tiền",
            },
            bulkPayment: {
                title: "Nhập thanh toán hàng loạt",
                button: "Nhập thanh toán",
                orderId: "Mã đơn hàng",
                amount: "Số tiền",
                paymentDate: "Ngày giao dịch",
                orderIdPlaceholder: "Nhập mã đơn hàng...",
                amountPlaceholder: "Nhập số tiền...",
                addRow: "Thêm dòng",
                rowCount: "{{count}} dòng có dữ liệu",
                noData: "Vui lòng nhập ít nhất một dòng dữ liệu",
                successCount: "Cập nhật thành công {{count}} đơn hàng",
                errorCount: "Lỗi cập nhật {{count}} đơn hàng",
            },
        },

        validation: {
            required: "Trường này là bắt buộc",
            invalidEmail: "Email không hợp lệ",
            fillAll: "Vui lòng điền đầy đủ thông tin!",
        },
        theme: {
            light: "Sáng",
            dark: "Tối",
            system: "Hệ thống",
            toggle: "Chuyển đổi giao diện",
        },
        dashboard: {
            title: "Dashboard",
            subtitle: "Tổng quan hệ thống quản lý phòng thí nghiệm",
            stats: {
                totalClients: "Tổng khách hàng",
                quotesThisMonth: "Báo giá tháng này",
                ordersInProgress: "Đơn hàng đang xử lý",
                revenueThisMonth: "Doanh thu tháng này",
                comparedToLastMonth: "so với tháng trước",
            },
            recentActivity: {
                title: "Hoạt động gần đây",
                newQuote: "Báo giá mới",
                orderCompleted: "Đơn hàng hoàn thành",
                newClient: "Khách hàng mới",
            },
            popularParameters: {
                title: "Chỉ tiêu phổ biến",
            },
        },
        organization: {
            name: "Tên tổ chức",
            address: "Địa chỉ",
            email: "Email",
            phone: "Số điện thoại",
            taxId: "Mã số thuế",
            data: {
                organizationName: "Viện nghiên cứu và phát triển sản phẩm thiên nhiên",
                address: "176 Phùng Khoang, Phường Đại Mỗ, Thành phố Hà Nội",
                email: "kiemnghiem@irdop.org",
                phone: "0943 863 355 - 0912 359 499",
                taxId: "0107149919",
            },
        },
        branch: {
            name: "Tên chi nhánh",
            address: "Địa chỉ",
            email: "Email",
            phone: "Số điện thoại",
            branches: [
                {
                    branchName: "VP1",
                    address: "12 Phùng Khoang 2, Trung Văn, Nam Từ Liêm, Hà Nội",
                    email: "",
                    phone: "",
                },
                {
                    branchName: "VP2",
                    address: "Lô I-4B-2.1, Đường N3 Khu Công nghệ cao TP. HCM Phường Long Thạnh Mỹ TP. Thủ Đức, TP. Hồ Chí Minh",
                    email: "",
                    phone: "",
                },
                {
                    branchName: "VP3",
                    address: "Số 015A Nguyễn Huệ - Phường Lào Cai - Tp. Lào Cai - Tỉnh Lào Cai",
                    email: "",
                    phone: "",
                },
            ],
        },
        sampleRequest: {
            header: "Phiếu gửi mẫu",
            title: "PHIẾU GỬI MẪU",
            order: "ĐƠN HÀNG:",
            section1: {
                title: "GỬI MẪU THỬ NGHIỆM (Phần dành riêng cho khách hàng)",
                title2: " THÔNG TIN THỂ HIỆN TRÊN KẾT QUẢ:",
                subtitle: "(Thông tin này sẽ được in trên Phiếu kết quả thử nghiệm và xác lập chủ sở hữu KQTN)",
            },
            section2: {
                title: " THÔNG TIN NGƯỜI LIÊN HỆ:",
                contactPerson: "Người liên hệ:",
            },
            section3: {
                title: " THÔNG TIN NHẬN KẾT QUẢ:",
                clientPhone: "SĐT:",
            },
            section4: {
                title: " THÔNG TIN XUẤT HÓA ĐƠN:",
                taxName: "Tên công ty:",
                contactPhone: "Số điện thoại (KH):",
                request: "Yêu cầu Viện IRDOP thử nghiệm theo các nội dung dưới đây:",
                quote: "(Trường hợp Khách hàng không xác định được phương pháp thử, vui lòng thống nhất với IRDOP)",
            },
            section5: {
                title: "GIẤY TỜ ĐI KÈM:",
            },
            institute: {
                name: "VIỆN NGHIÊN CỨU VÀ PHÁT TRIỂN SẢN PHẨM THIÊN NHIÊN",
                address: "Add: 12 Phùng Khoang 2, P. Đại Mỗ, TP. Hà Nội",
                tel: "Tel: 024 355 35 355",
                email: "Email: cskh@irdop.org",
            },
            sampleInfo: {
                sampleName: "Tên mẫu",
                lotNo: "Số lô",
                mfgDate: "Ngày sản xuất",
                expDate: "Hạn sử dụng",
                placeOfOrigin: "Nơi sản xuất",
                productionAddress: "Địa chỉ sản xuất",
            },
            signer: {
                customer: "KHÁCH HÀNG",
                confirm: "Tôi xác nhận đã đọc và đồng ý với các quy định chung đính kèm",
                signNote: "(Ký, ghi rõ họ tên, đóng dấu)",
                receiptTitle: "BIÊN NHẬN",
                labOnly: "Phần dành riêng cho PTN",
                receivedDate: "Ngày nhận mẫu:",
                receivedLocation: "Địa điểm nhận mẫu:",
                atInstitute: "Tại Viện IRDOP",
                other: "Khác:",
                retention: "Lưu mẫu:",
                noRetention: "Không lưu mẫu",
                retainSample: "Lưu mẫu",
                receiverTitle: "NGƯỜI NHẬN MẪU",
                receiver: "NGƯỜI NHẬN MẪU",
            },

            rules: {
                header: {
                    title: "QUY ĐỊNH CHUNG",
                    attached: "(Kèm theo Phiếu gửi mẫu",
                    archived: "bản điện tử dự trữ tại https://irdop.org/quydinhchung/)",
                },
                items: [
                    "Chủ sở hữu Kết quả thử nghiệm (KQTN) là tên khách hàng được thể hiện trên KQTN, chỉ chủ sở hữu mới có quyền yêu cầu điều chỉnh hay thay đổi thông tin hợp lý trên KQTN theo quy định của IRDOP.",
                    "Trong trường hợp Khách hàng không ghi tên phương pháp thử nghiệm, không xác định được phương pháp thử nghiệm thì phải thống nhất với IRDOP và ghi dòng chữ “Đã thống nhất với IRDOP về phương pháp thử” trong phiếu gửi mẫu.",
                    "Chỉ tiêu thử nghiệm có thể phải sử dụng nhà thầu phụ trong trường hợp cần thiết, IRDOP sẽ thông báo cho Khách hàng trước khi áp dụng.",
                    "Phiếu KQTN do IRDOP phát hành đến Khách hàng chỉ có giá trị trên đối tượng thử nghiệm tại thời điểm thử nghiệm và không làm giảm trách nhiệm cũng như nghĩa vụ thuộc hợp đồng thương mại hoặc trách nhiệm pháp lý của Khách hàng hay bên cung cấp đối tượng thử nghiệm. Việc trích sao một phần Phiếu KQTN chỉ được thực hiện khi có sự đồng ý bằng văn bản của IRDOP; mọi hành vi vi phạm quy định này, khách hàng phải tự chịu trách nhiệm theo quy định pháp luật.",
                    "Khách hàng không được tự ý sửa đổi Phiếu KQTN của IRDOP dưới bất cứ hình thức nào; mọi hành vi vi phạm quy định này, khách hàng phải tự chịu trách nhiệm theo quy định pháp luật.",
                    "IRDOP không chịu trách nhiệm về các thông tin do Khách hàng cung cấp tại Phiếu gửi mẫu, bao gồm nhưng không giới hạn: tên mẫu, ký hiệu, nguồn gốc mẫu, tên khách hàng, các chỉ tiêu yêu cầu thử nghiệm.",
                    "Trong trường hợp việc thử nghiệm có trở ngại hay cần thay đổi, hai bên sẽ thông báo, bàn bạc để cùng nhau giải quyết.",
                    "IRDOP thực hiện đúng nội dung thử nghiệm đã thỏa thuận với Khách hàng dựa trên kiến thức, năng lực kỹ thuật hiện có để cung cấp tốt nhất dịch vụ thử nghiệm cho Khách hàng với đầy đủ trách nhiệm của mình theo đúng các quy định hiện hành.",
                    "IRDOP giữ quyền từ chối yêu cầu thử nghiệm khi IRDOP không nhận đủ thông tin hoặc thông tin không xác thực về đối tượng thử nghiệm hoặc khi đối tượng thử nghiệm không đáp ứng yêu cầu thử nghiệm. IRDOP không nhận khiếu nại trong trường hợp không có mẫu lưu hoặc hết thời hạn lưu theo mẫu quy định.",
                    "IRDOP thực hiện bảo mật kết quả thử nghiệm cho Khách hàng, không cung cấp thông tin về công việc thử nghiệm cho bên thứ ba nào biết nếu không có sự đồng ý bằng văn bản của Khách hàng, ngoại trừ các quy định khác của pháp luật.",
                ],
                contact: {
                    title: "Thông tin liên hệ:",
                    orgName: "Viện nghiên cứu và phát triển sản phẩm thiên nhiên (IRDOP)",
                    address: "Địa chỉ: 12 Phùng Khoang 2 – Phường Đại Mỗ - TP. Hà Nội",
                    phone: "SĐT: 024 355 35 355",
                    email: "Email: Cskh@irdop.org",
                },
            },

            //   section1: {
            //     title: "1. Thông tin:",
            //     clientInfo: "1.1 Thông tin khách hàng:",
            //     contactInfo: "1.2 Thông tin người liên hệ:",
            //   },
            //   section2: {
            //     title: "2. Nơi nhận kết quả:",
            //   },
            //   section3: {
            //     title: "3. Thông tin đăng ký thử nghiệm (mẫu):",
            //   },
            //   section4: {
            //     title: "4. Thông tin liên hệ:",
            //   },
            clientName: "Tên khách hàng:",
            address: "Địa chỉ:",
            phone: "Số điện thoại:",
            contactPhone: "Điện thoại:",
            invoiceInfo: "Thông tin xuất hóa đơn (nếu khác thông tin trên):",
            taxId: "MST/CCCD:",
            invoiceEmail: "Email HĐ:",
            contactPerson: "Tên người liên hệ:",
            identity: "CCCD:",
            email: "Email:",
            hardCopy: "Bản cứng:",
            softCopy: "Bản mềm (Email):",
            table: {
                sampleDesc: "Mô tả mẫu",
                parameters: "Chỉ tiêu thử nghiệm",
            },
            note1: "Mẫu gửi bắt buộc phải có tên mẫu, mô tả mẫu.",
            note2: "Thông tin mẫu trên phiếu gửi mẫu phải khớp với thông tin thực tế trên mẫu.",
            note3: "Khách hàng đồng ý với phương pháp thử do IRDOP tự lựa chọn nếu không có yêu cầu cụ thể.",
            note4: "Không nhận khiếu nại trong trường hợp không có mẫu lưu hoặc hết thời hạn lưu theo quy định.",
            disclaimer:
                "Khách hàng hiểu rằng các kết quả thử nghiệm và phiếu kết quả do phòng thử nghiệm cung cấp dựa trên mẫu gửi chỉ được sử dụng cho các mục đích hợp pháp, phù hợp với quy định pháp luật hiện hành, và phòng thử nghiệm không chịu trách nhiệm đối với bất kỳ việc sử dụng kết quả thử nghiệm ngoài mục đích đã thỏa thuận hoặc các chỉ tiêu không thuộc phạm vi công nhận (trừ trường hợp do lỗi của phòng thử nghiệm) và sẽ không có khiếu nại.",
            receiver: "Người tiếp nhận",
            sign1: "Ký, ghi rõ họ tên",
            customer: "Khách hàng",
            sign2: "Ký, đóng dấu, ghi rõ họ tên",
        },
        file: {
            upload: "Tải tệp lên",
            uploading: "Đang tải tệp...",
            uploadSuccess: "Tải tệp thành công",
            uploadError: "Lỗi tải tệp",
            delete: "Xóa tệp",
            deleteConfirm: "Bạn có chắc chắn muốn xóa tệp này?",
            deleteSuccess: "Xóa tệp thành công",
            noFiles: "Chưa có tài liệu đính kèm",
            view: "Xem tệp",
            download: "Tải xuống",
            attachments: "Tài liệu đính kèm",
            invalidType: "Loại file không được hỗ trợ",
            sizeTooLarge: "Dung lượng file quá lớn (tối đa 20MB)",
            viewDownload: "Xem/Tải xuống",
            attachSuccess: "Đã cập nhật danh sách tài liệu",
            attachError: "Không thể cập nhật danh sách tệp đính kèm vào đơn hàng",
            updateError: "Lỗi khi cập nhật đơn hàng",
            notFoundUrl: "Không tìm thấy đường dẫn tải file",
            downloadError: "Lỗi khi tải file",
            deleteFromOrderSuccess: "Đã xóa tài liệu khỏi đơn hàng",
            deleteFromOrderError: "Lỗi khi xóa tài liệu",
            connectionError: "Lỗi kết nối",
            uploadSuccessCount: "Đã tải lên thành công {{count}} tệp tin",
            uploadLimitError: "Tổng dung lượng file không được vượt quá 25MB",
            uploadingCount: "Đang tải lên...",
            confirmUpload: "Xác nhận tải lên",
            readyToUpload: "Đã sẵn sàng tải lên",
            noFileSelected: "Chưa chọn tệp nào",
            uploadTitle: "Tải lên tài liệu",
            uploadSubtitle: "Chọn phân loại và tải tệp lên đơn hàng",
            category: "Phân loại tài liệu",
            selectToUpload: "Nhấn vào để chọn tệp",
            maxSizeLimit: "Dung lượng tối đa cho phép: 25MB",
            selectedFilesCount: "Danh sách tệp đã chọn ({{count}})",
            tags: {
                INVOICE: "Hóa đơn",
                ORDER_FORM: "Đơn hàng",
                SAMPLE_SUBMISSION: "Phiếu gửi mẫu",
                PRODUCT_SPEC: "Tiêu chuẩn cơ sở",
                SAMPLE_IMAGE: "Ảnh mẫu",
                Standard: "Tiêu chuẩn cơ sở",
                Contract: "Hợp đồng",
                Payment: "Thanh toán",
                SampleImage: "Ảnh mẫu",
            },
        },
    },
};
