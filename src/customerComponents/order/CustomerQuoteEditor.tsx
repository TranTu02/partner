...
export const CustomerQuoteEditor = forwardRef<CustomerQuoteEditorRef, CustomerQuoteEditorProps>(
    ({ mode, initialData, onSaveSuccess }, ref) => {
        ...
        const handleRemoveSample = (id: string) => { if (!isReadOnly) setSamples(samples.filter((s) => s.id !== id)); };
        const handleUpdateSample = (id: string, updates: Partial<SampleWithQuantity>) => { if (!isReadOnly) setSamples(samples.map((s) => s.id === id ? { ...s, ...updates } : s)); };
        ...
        const handleAddAnalyses = (items: Matrix[]) => {
            ...
            const newAnalyses: AnalysisWithQuantity[] = items.map((m, i) => {
                ...
                return { ...m, id: `A${ts}_${i}_${m.matrixId}`, unitPrice, taxRate, feeAfterTax, quantity: 1, groupId: (m as any).groupId, groupDiscount: (m as any).groupDiscount, discountRate: (m as any).discountRate };
            });
        ...
        const handleSave = async () => {
            ...
            try {
                const response = mode === "create"
                    ? await customerCreateQuote({ body: quoteData })
                    : await customerUpdateQuote({ body: quoteData });
                ...
                samples: samples.map((s) => {
                    ...
                    return {
                        ...
                        analyses: s.analyses.map((a) => {
                            ...
                            return { parameterName: a.parameterName, parameterId: a.parameterId, feeBeforeTax: lineNet, feeBeforeTaxAndDiscount: lineList, taxRate: tr, feeAfterTax: lineNet * (1 + tr / 100), discountRate: dr, quantity: qty, unitPrice: up };
                        }),
                    };
                }),
                pricing,
